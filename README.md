...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>mypy-type-checker.enabled</td>
      <td><code>true</code></td>
      <td>Enable/disable linting Python files with Mypy. This setting can be applied globally or at the workspace level. If disabled, the linting server itself will continue to be active and monitor read and write events, but it won't perform linting.</td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td>Whether the Mypy daemon (<code>dmypy</code>) will take precedence over <code>mypy</code> for type checking. Note: if <code>mypy-type-checker.reportingScope</code> is set to <code>workspace</code>, enabling the Mypy daemon will offer a faster type checking experience. This setting will be overridden if <code>mypy-type-checker.path</code> is set.
    </tr>
    <tr>
